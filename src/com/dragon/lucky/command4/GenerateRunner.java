...
//        Log.i("单次计算结果数：" + data.size());
    ...
    private void printSingleData(List<ResultBean> data) {
        List<ResultBean> print = new ArrayList<>();
        String output = mCommand.getOutputPath().split("\\.")[0] + "-" + currentGenerateIdx + ".txt";
        int existsCount = 0;
        for (int i = 0; i < data.size(); i++) {
            ...
            if (exists) {
                existsCount++;
                print.add(d);
                Log.i("第" + (i + 1) + "行存在对应的数据！存在：" + existsCount + " 条");
            }
        }
        Log.i("校验完毕，结束！");
        try {
            FileReadHelper.writeToFile(output, print);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }

    private void printAllData(List<ResultBean> data) {
        String output = mCommand.getOutputPath().split("\\.")[0] + "-all-" + currentGenerateIdx + ".txt";
        try {
            FileReadHelper.writeToFile(output, data);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }

    private void printTjData(List<ResultBean> data) {
        String output = mCommand.getOutputPath().split("\\.")[0] + "-tj-" + currentGenerateIdx + ".txt";
        try {
            FileReadHelper.writeToFile(output, data);
        } catch (IOException e) {
            e.printStackTrace();
    ...
    private void firstFilterData(List<ResultBean> previewResult) {
        ...
        filterData.addAll(result.subList(ePercent - 1, result.size()));

        printTjData(filterData);
...
//                    Log.i("percent.getFootPercent() = " + percent.getFootPercent());
                    filterData.addAll(tempData.subList(sPercent - 1, Math.min(ePercent + 1, tempData.size())));//拿到中间部分
                } else {
