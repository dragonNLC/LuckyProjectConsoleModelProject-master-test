...
//                    String count = contents[1].replace("[", "").replace("]", "");
                    ...
                    if (d.contains("],[")) {
                        ...
                        count = content[1].replace("[", "").replace("]", "");
                    } else {
...
//            Log.i("generateData.size() = " + generateData.size());
            if (generateData.size() == 1) {
                printData(generateData.get(0).getData());
                return;
            } else {
...
//                    Log.i("tempBaseData2.getAppendData().size() = " + tempBaseData2.getAppendData().size());
        ...
        if (fullDataAppend.size() == 0) {//升位失败
            ...
            //开始检查得到的五位里面中4\5位的个数
            List<GenerateBaseDataBean.AppendData> existsData1 = printSingleDataIdx(fullDataAppend, 1);//先得到符合的数
            List<GenerateBaseDataBean.AppendData> existsData2 = printSingleDataIdx(fullDataAppend, 2);//先得到符合的数
            List<GenerateBaseDataBean.AppendData> existsData3 = printSingleDataIdx(fullDataAppend, 3);//先得到符合的数
            ...
            List<GenerateBaseDataBean.BaseData> existsData1_2 = printSingleDataIdx2(notFullAppendData, 1);//先得到符合的数
            List<GenerateBaseDataBean.BaseData> existsData2_2 = printSingleDataIdx2(notFullAppendData, 2);//先得到符合的数
            List<GenerateBaseDataBean.BaseData> existsData3_2 = printSingleDataIdx2(notFullAppendData, 3);//先得到符合的数
            ...
            try {
                FileReadHelper.writeToFile4(outputFilePath, outputFilePath2, fullDataAppend, notFullAppendData, existsData1.size(), existsData2.size(), existsData3.size(), existsData4.size(), existsData5.size(),
                        existsData1_2.size(), existsData2_2.size(), existsData3_2.size(), existsData4_2.size(), existsData5_2.size());
            } catch (IOException e) {
                e.printStackTrace();
            }
