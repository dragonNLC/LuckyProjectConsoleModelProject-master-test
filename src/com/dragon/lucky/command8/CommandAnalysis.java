    ...
    public void analysisCommand(String[] args) {
        if (args != null && args.length > 0) {
            ...
            for (int i = 0; i < args.length; i++) {
                ...
                switch (arg) {
                    case "-i"://输入原始数据
                        command.getInputPath().add(args[++i]);
                        break;
                    case "-o"://输出结果文件
                        command.setOutputPath(args[++i]);
                        break;
                    case "-f"://输出结果文件
                        command.getFilterPath().add(args[++i]);
                        break;
                    case "-m"://输入预加载文件
    ...
    public void analysisCommand(String[] args, CommandBean commandBean) {
        if (args != null && args.length > 0) {
            for (int i = 0; i < args.length; i++) {
                ...
                switch (arg) {
                    case "-i"://输入原始数据
                        commandBean.getInputPath().add(args[++i]);
                        break;
                    case "-o"://输出结果文件
                        commandBean.setOutputPath(args[++i]);
                        break;
                    case "-f"://输出结果文件
                        commandBean.getFilterPath().add(args[++i]);
                        break;
                    case "-m"://输入预加载文件
            ...
            if (scanner.hasNext()) {
                String commands = scanner.nextLine();
                String[] commandSplit = commands.split(" ");
                analysisCommand(commandSplit, commandBean);
                scanner.close();
            }
        }
    }
