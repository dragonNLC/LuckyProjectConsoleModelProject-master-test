    ...
    public void run(CommandBean commandBean) throws IOException {
        if (!Utils.isEmpty(CommandAnalysis.getInstance().getCommand().getCommandsPath())) {
            ...
            if (!Utils.isEmpty(commandBean.getCommandsPath())) {
                ...
                new Thread(() -> {
                    try {
                        GenerateRunner2.getInstance().run(singleCommand, ProcessRunner.this);
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
                }).start();
            ...
            GenerateRunner2.getInstance().run(commandBean, ProcessRunner.this);
        }
    }
...
//        System.gc();
        ...
        if (mCommands != null && mCommandIdx < mCommands.size()) {
            ...
            ExcelUtils.outputFile(path, name, generateData);
        }
    }
