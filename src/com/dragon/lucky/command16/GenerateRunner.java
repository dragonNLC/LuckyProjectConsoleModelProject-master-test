...
//                List<NumberDataBean> firstFilterDatas = new ArrayList<>(baseFirstFilterDatas);

                int id = (int) (Math.random() * tempPreviewResult.size());
//                Log.i("tempPreviewResult = " + tempPreviewResult.size());
...
//                            Log.i("count = " + count + "   result.size()  = " + result.size() + "   template = " + template.size());
                ...
                /*} else if (mCommand.getType() == CommandBean.TYPE_2) {
                    while (true) {
                        Iterator<NumberDataBean> dataBeanIterator = tempPreviewResult.iterator();
                        while (dataBeanIterator.hasNext()) {
                            NumberDataBean ndb = dataBeanIterator.next();
                            int replyCount = 0;
                            breakPoint:
                            for (int k = 0; k < ndb.getData().length; k++) {
                                for (int j = 0; j < result.size(); j++) {
                                    for (int l = 0; l < result.get(j).getData().length; l++) {
                                        if (ndb.getData()[k] == result.get(j).getData()[l]) {
                                            replyCount++;
//                                        Log.i("replyCount = " + replyCount);
                                            if (replyCount > mCommand.getGenerateReplySize()) {//超出相同数字范围的，移除
//                                            Log.i("replyCount remove ");
                                                dataBeanIterator.remove();
                                                break breakPoint;
                                            }
                                        }
                        ...
                        if (tempPreviewResult.size() > 0) {
                            id = (int) (Math.random() * tempPreviewResult.size());
                            firstFilterData = tempPreviewResult.remove(id);
//                        tempPreviewResult.remove(firstFilterData);
                            result.add(firstFilterData);
                            if (result.size() >= mCommand.getLines()) {//已经生成足够的行数了
                                break;
