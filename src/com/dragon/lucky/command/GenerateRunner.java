...
public class GenerateRunner {
    ...
    private List<ResultBean> compileData = new ArrayList<>();
...
//                    Log.i("line = " + splitLine.length);
    ...
    ///////////////////////////////////////////////
    private List<ResultBean> generateResults(BaseGenerateNumberBean bgn, List<ResultBean> result) {
...
//        Log.e("TAG", "result = " + result.size());
        ...
        if (controls.size() > 0) {
            ...
            if (size > 0) {
                ...
                FilterUtils.filterResultBean(result, controls, 0, resultBeans);
                resultBeans.sort(Comparator.comparingLong(ResultBean::getId));
                try {
                    FileReadHelper.writeToFile(mCommand.getOutputPath(), resultBeans);
                } catch (IOException e) {
                    e.printStackTrace();
                }
...
//            }
        ...
        for (int j = 0; j < cutResultBeans.size(); j++) {
            threadCount++;
            new ContainThread(this, data, cutResultBeans.get(j).start, cutResultBeans.get(j).end, data2, cutResultBeans2.get(j).start, cutResultBeans2.get(j).end)
                    .start();
        }
    }
...
//            CutResultBean2 cutBean = new CutResultBean2(j * size, Math.min((j + 1) * size, data2.size()));
    ...
    public synchronized void containCompileData(List<ResultBean> data) {
...
//            Log.i("threads.size()：" + threads.size());
            if (currentFilterThreadId >= threads.size()) {//最后一个了
                ...
                try {
                    FileReadHelper.writeToFile(mCommand.getOutputPath(), compileData);
                } catch (IOException e) {
                    e.printStackTrace();
                }
            } else {//执行下一次结果
...
//                    }
//                }
                List<ResultMergeBean> collectMergeBeans = FilterUtils.collectMergeBean(compileData);
//                if (currentFilterThreadId >= 5 && currentFilterThreadId < 9) {
