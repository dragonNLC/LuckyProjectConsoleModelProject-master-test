...
public class FilterUtils {
    ...
    //取得数据然后取并集
    public static void filterResultBean(List<ResultBean> base, List<BaseControlNumberBean> filter, int position, List<ResultBean> resultData) {
        if (position == filter.size()) {
            /*for (int i = 0; i < base.size(); i++) {
                if (!resultData.contains(base.get(i))) {//数组里面不存在的，才合并在一起
                    resultData.add(base.get(i));
                }
            }*/
...
//        Log.i("第：" + position + "条规则；");
        ...
        for (int j = 0; j < printRules.size(); j++) {
            ...
            //得到符合规则的数据
            List<ResultBean> result = getContains(base, bcn.getBaseNumber(), rule);
...
//        Log.i("筛选次数：" + FILTER_COUNT);
    ...
    public static List<ResultBean> getContains(List<ResultBean> data, List<Byte> baseData, int rule) {
...
//            Log.i("containCount = " + containCount + "    rule = " + rule);
    ...
    public static List<ResultMergeBean> collectMergeBean(List<ResultBean> data) {
        List<ResultMergeBean> mergeBeans = new ArrayList<>();
        //合并前n位数为同一个数组
        Map<String, ResultMergeBean> mergeBeanMap = new HashMap<>();
        for (int i = 0; i < data.size(); i++) {
            ResultBean rb = data.get(i);
...
//                Log.i("mergeArr == null：" + rb.getMergeArr() + "   ");
    ...
    public static List<ResultBean> getContains2(String name, List<ResultMergeBean> data, int start, int end, List<ResultMergeBean> data2, int start2, int end2) {
...
//                            if (children3.getId() == children4.getId()) {
                            if (Arrays.equals(children3.getData(), children4.getData())) {
